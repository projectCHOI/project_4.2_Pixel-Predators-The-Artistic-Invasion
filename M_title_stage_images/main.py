...
# 플레이어 설정
player_speed = 200  # 속도 조정 (초당 픽셀 수)
...
invincible_elapsed_time = 0  # 무적 시간 누적 변수 초기화

# 충돌 효과 설정
collision_effect_start_time = 0
collision_image = None
collision_effect_duration = 0
collision_elapsed_time = 0  # 충돌 효과 시간 누적 변수 초기화

# 공격 설정
attacks = []
attack_speed = 500  # 초당 픽셀 수
...
# 폭탄 적 생성 시간 누적 변수 초기화
bomb_elapsed_time = 0

# 스피드 아이템 시간 누적 변수 초기화
speed_item_elapsed_time = 0
...
def generate_enemies(level):
    ...
    for _ in range(num_enemies):
        ...
        if size == 40:
            ...
            enemies.append([pos, size, enemy_type, direction_normalized, speed, target_pos, 0, image, speed, 0])  # time_since_last_attack 추가
            continue
        elif size == 20:
            enemy_type = "approach_and_shoot"

        enemies.append([pos, size, enemy_type, direction, speed, None, 0, image, speed, 0])  # time_since_last_attack 추가
...
def add_bomb_enemy():
    ...
    length = math.hypot(direction_vector[0], direction_vector[1])
    direction_normalized = [direction_vector[0] / length, direction_vector[1] / length]
    enemies.append([pos, size, "bomb", direction_normalized, 90, None, 0, enemy_bomb_image, 90, 0])  # time_since_last_attack 추가

# 대시보드 그리기 함수
def draw_dashboard(elapsed_stage_time):
    # 플레이 시간 표시
    time_text = font.render(f"Time: {int(elapsed_stage_time)}s", True, WHITE)
...
def draw_objects(player_pos, enemies, background_image, mouse_pos, elapsed_stage_time, collision_image=None, speed_item_pos=None, power_item_pos=None, heal_item_pos=None, heal_item_image=None):
    ...
    # 적 그리기
    for enemy in enemies:
        enemy_pos, enemy_size, enemy_type, _, _, _, _, enemy_image, _, _ = enemy
        win.blit(enemy_image, (int(enemy_pos[0]), int(enemy_pos[1])))
    # 아이템 및 플레이어 그리기
    if speed_item_pos:
        win.blit(speed_item_image, (int(speed_item_pos[0]), int(speed_item_pos[1])))
    if power_item_pos:
        win.blit(power_item_image, (int(power_item_pos[0]), int(power_item_pos[1])))
    if heal_item_pos and heal_item_image:
        win.blit(heal_item_image, (int(heal_item_pos[0]), int(heal_item_pos[1])))
    win.blit(player_image, (int(player_pos[0]), int(player_pos[1])))
    if collision_image:
        win.blit(collision_image, (int(player_pos[0]), int(player_pos[1])))
    # 에너지 볼 그리기
    ...
    # 마우스 위치 그리기
    pygame.draw.circle(win, RED, (int(mouse_pos[0]), int(mouse_pos[1])), 5)
    # 대시보드 그리기
...
#!                    start_ticks = pygame.time.get_ticks()  # 게임 시작 시간 기록 (항상 초기화)
                    ...
                    enemies = []
                    total_seconds = 0  # 전체 게임 시간 초기화
                    elapsed_stage_time = 0  # 스테이지 경과 시간 초기화
                    invincible_elapsed_time = 0  # 무적 시간 초기화
                    collision_elapsed_time = 0  # 충돌 효과 시간 초기화
                    bomb_elapsed_time = 0  # 폭탄 적 생성 시간 초기화
                    speed_item_elapsed_time = 0  # 스피드 아이템 시간 초기화
        ...
        # bomb 적 생성
        bomb_elapsed_time += delta_time
        if level in bomb_stages and bomb_elapsed_time >= bomb_appear_interval / 1000:
            add_bomb_enemy()
            bomb_elapsed_time = 0
        ...
        # 보스가 활성화된 경우 처리
        if boss.boss_active:
            boss.move(delta_time)  # 보스 이동
            boss.attack()  # 보스 공격
            if boss.update_attacks(player_pos, delta_time):  # 보스의 공격과 플레이어의 충돌 체크
                current_health -= 1
                if current_health <= 0:
                    game_active = False
                    game_over = True
        ...
        # 적 이동 및 행동 처리
        for enemy in enemies:
            pos, size, enemy_type, direction, speed, target_pos, shots_fired, enemy_image, original_speed, time_since_last_attack = enemy
            # time_since_last_attack 업데이트
            enemy[9] += delta_time
            if enemy_type == "move_and_disappear":
                ...
                if target_pos:
                    ...
                    if distance_to_target > speed * delta_time:
                        direction_normalized = [direction[0], direction[1]]
                ...
                if action_time < 2.5:
                    ...
                    pos[0] += direction_vector[0] * speed * delta_time
                    pos[1] += direction_vector[1] * speed * delta_time
        ...
        for attack in attacks:
            ...
            if length == 0:
                continue
            direction = (direction[0] / length * attack_speed * delta_time, direction[1] / length * attack_speed * delta_time)
        ...
        new_enemies = []
        for enemy in enemies:
            enemy_pos, enemy_size, _, _, _, _, _, enemy_image, _, _ = enemy
            hit = False
            for attack in attacks:
        ...
        if invincible:
            ...
            if invincible_elapsed_time >= invincible_duration / 1000:
                invincible = False
                invincible_elapsed_time = 0

        # 충돌 이미지 표시 시간 체크
        if collision_image:
            collision_elapsed_time += delta_time
            if collision_elapsed_time >= collision_effect_duration / 1000:
                collision_image = None
                collision_elapsed_time = 0
        ...
        if speed_item_pos:
            ...
            if player_rect.colliderect(speed_rect):
                ...
                speed_item_elapsed_time = 0  # 스피드 아이템 시간 누적 변수 초기화
                # 적들의 속도를 감소
                for enemy in enemies:
                    enemy[4] = 70  # 속도를 70으로 설정
                speed_item_pos = None

        # 스피드 아이템 지속시간 체크
        if speed_item_active:
            speed_item_elapsed_time += delta_time
            if speed_item_elapsed_time >= speed_item_duration / 1000:
                speed_item_active = False
                speed_item_elapsed_time = 0
                # 적들의 속도를 원래 속도로 복원
                for enemy in enemies:
        ...
        new_energy_balls = []
        for ball in energy_balls:
            ball[0] += ball[3][0] * 200 * delta_time  # x 좌표 업데이트
            ball[1] += ball[3][1] * 200 * delta_time  # y 좌표 업데이트
