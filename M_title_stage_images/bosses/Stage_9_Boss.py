...
class Stage9Boss:
    def __init__(self):
        ...
        self.minion_image = load_image("boss_skilles", "boss_stage9_N.png", size=(60, 60))  # 미니언 이미지
        self.attack_image = load_image("boss_skilles", "boss_stage90_b.png", size=(60, 60))  # 보스 공격 이미지
    ...
    def draw_attacks(self, win):
        for attack in self.boss_attacks:
            rotated = pygame.transform.rotate(self.attack_image, attack.get("angle", 0))
    ...
    def check_energy_ball_collision(self, ball_pos, player_pos):
        bx, by = ball_pos
        px, py = player_pos
        if px < bx < px + 50 and py < by < py + 50:
            return True
        return False
