...
class Stage6Boss:
    def __init__(self):
        # 이미지 로드 (철 4개 필요: 보스 이미지, 공격 이미지 3개, 보석 이미지)
        ...
        self.boss_damage = 2
        self.boss_pos = [640 - 75, 360 - 75]  # 화면 정중안
        ...
        if self.boss_hp > 0:
            ...
            if self.boss_hit:
                if current_time - self.boss_hit_start_time >= self.invincible_duration:
                    self.boss_hit = False  # 무적 상태 및 꺘벝임 종료
    ...
    def draw_health_bar(self, win, font):
        if self.boss_active and self.boss_hp > 0:
            ...
            health_bar_y = 680
            health_bar_width = 200  # 체력 바의 체 너비를 200으로 설정
    ...
    def check_hit(self, attacks):
        ...
        for attack in attacks:
            ...
            if self.check_attack_collision(attack_start, attack_end, self.boss_pos, 150):
                self.boss_hp -= 1  # 데미지 적역
                if self.boss_hp < 0:
                    self.boss_hp = 0  # 체력이 음수가 되지 않도록
                self.boss_hit = True  # 보스가 공격을 받았을 표시
